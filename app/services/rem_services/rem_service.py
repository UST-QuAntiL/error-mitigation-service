    ...
    if application_method.requires == MatrixType.mm:
        ...
        if mitigator is not None:
            sorted_counts = sort_dict_by_qubitorder(
                request.counts, request.qubits, metadata["qubits"]
            )
            return application_method.appyl_mitigation(
                mitigator, counts=sorted_counts, qubits=request.qubits
            )
        ...
        if cm is not None:
            ...
            return application_method.appyl_mitigation(
                cm, counts=sorted_counts, qubits=request.qubits
            )
        elif request.credentials and request.provider:
